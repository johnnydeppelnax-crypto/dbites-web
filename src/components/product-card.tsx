...
const categoryColors: Record<string, { bg: string; text: string; gradient: string }> = {
  ...
  Mixes: { bg: 'bg-cyan-100/80', text: 'text-cyan-700', gradient: 'from-cyan-500 to-blue-500' },
}
        ...
        <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-orange-50 to-cyan-50">
          ...
          {product.featured && (
            <div className="absolute top-2 sm:top-3 left-2 sm:left-3">
              <Badge className="bg-gradient-to-r from-orange-500 to-amber-500 text-white border-0 shadow-lg shadow-orange-500/25 text-[9px] sm:text-xs font-bold px-2 sm:px-3 py-0.5 sm:py-1">
                Featured
              </Badge>
            </div>
          )}
          ...
          <div className="absolute top-2 sm:top-3 right-2 sm:right-3">
            <span className={`${colors.bg} ${colors.text} border border-white/50 text-[9px] sm:text-[10px] font-bold px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full backdrop-blur-sm shadow-sm`}>
              {product.category}
            </span>
          </div>
          ...
          {!product.inStock && (
            <div className="absolute inset-0 bg-white/70 backdrop-blur-sm flex items-center justify-center">
              <span className="text-xs sm:text-sm font-bold bg-gradient-to-r from-gray-700 to-gray-900 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full">Out of Stock</span>
            </div>
          )}
        </div>

        <CardContent className="p-3 sm:p-4">
          <h3 className="font-bold text-xs sm:text-sm line-clamp-1 group-hover:text-orange-600 transition-colors mb-1">
            {product.name}
          </h3>
          <p className="text-[10px] sm:text-xs text-foreground/50 line-clamp-2 mb-2 sm:mb-3 leading-relaxed">
            {product.description}
          </p>
          <div className="flex items-center justify-between">
            <div>
              <span className={`text-base sm:text-lg font-black bg-gradient-to-r ${colors.gradient} bg-clip-text text-transparent`}>
                ${product.price.toFixed(2)}
              </span>
              <span className="text-[10px] sm:text-xs text-foreground/40 ml-1">/ {product.weight}</span>
            </div>
            <div className="flex items-center gap-1 bg-amber-50 px-1.5 sm:px-2 py-0.5 rounded-full border border-amber-100/50">
              ...
              <span className="text-[10px] sm:text-xs font-bold text-amber-700">{product.rating}</span>
            </div>
          </div>
        </CardContent>
      </Card>
